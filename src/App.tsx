...
type RMCharacter = { name: string, image: string, origin: { name: string } }
...
function App() {
    ...
    const [allCharacters, setAllCharacters] = useState(rMJson.results)
    const [searchText, setSearchText] = useState("")

    function onChange(event: ChangeEvent<HTMLInputElement>): void {
        setSearchText(event.target.value)
        setAllCharacters(rMJson.results.filter(c => c.name.toLowerCase().includes(event.target.value.toLowerCase())))
    }

    return (
        <div className="App">
            <h1>Rick & Morty App</h1>
            <ActionBar text={searchText} onChangeFunction={onChange}/>
            <CharacterGallery characters={allCharacters}/>
        </div>
    );
}
